...
test('canonical V7 bridge keeps untrusted PR request checkout free of private credentials', () => {
  ...
  assert.match(privateLane, /AUDIT_CONTROLLER_TOKEN_PRIMARY:\s*\$\{\{ secrets\.AUDIT_CONTROLLER_GITHUB_TOKEN \}\}/);
  assert.match(privateLane, /AUDIT_CONTROLLER_TOKEN_FALLBACK:\s*\$\{\{ secrets\.PREFLIGHTSIM_GITHUB_TOKEN \}\}/);
  assert.match(privateLane, /GH_TOKEN="\$AUDIT_CONTROLLER_TOKEN_PRIMARY" gh api repos\/CurveYield2\/Audit-Controller --silent/);
  assert.match(privateLane, /GH_TOKEN="\$AUDIT_CONTROLLER_TOKEN_FALLBACK" gh api repos\/CurveYield2\/Audit-Controller --silent/);

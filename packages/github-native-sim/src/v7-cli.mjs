...
async function appendGithubEnv(values) {
  ...
  if (!target) return;
  const body = Object.entries(values).map(([key, value]) => `${key}=${String(value)}\n`).join('');
  await fs.appendFile(target, body);
}

async function readRequest(requestPath) {
  if (!requestPath) throw new Error('Missing --request <request.json>');
  const bytes = await fs.readFile(path.resolve(requestPath));
  let parsed;
...
async function executeCommand(args) {
  const requestPath = args.request ?? '.v7-request/request.json';
  const { request, bytes: requestBytes } = await readRequest(requestPath);
  const evidenceRoot = path.resolve(args['evidence-dir'] ?? V7_POLICY.evidence.root, request.requestId);
  const workspaceRoot = path.resolve(args.workspace ?? '.audit-work/v7-execution');
  await fs.rm(evidenceRoot, { recursive: true, force: true });
  await fs.mkdir(evidenceRoot, { recursive: true });

  const runnerCommit = process.env.GITHUB_SHA ?? null;
  ...
  const rawPath = path.join(evidenceRoot, 'raw-result.json');
  await fs.writeFile(rawPath, raw);
  const artifactDigest = sha256(Buffer.from(raw));
  const requestTransportSha256 = sha256(requestBytes);
  const evidence = {
    schemaVersion: V7_POLICY.evidence.schemaVersion,
    requestId: request.requestId,
    requestDigest: request.requestDigest,
    requestTransportSha256,
    processId: request.processId,
    phaseId: request.phaseId,
    profileId: request.profileId,
    source: request.source,
    runnerImplementation: {
      repository: process.env.GITHUB_REPOSITORY ?? 'CurveYield2/Contract-Automation',
      commit: runnerCommit,
      manifest: 'process/RUNNER_MANIFEST.json',
    },
  ...
  await fs.writeFile(path.join(evidenceRoot, 'execution-summary.json'), `${JSON.stringify(summary, null, 2)}\n`);

  const artifactName = `v7-${safeArtifactPart(request.requestId)}-${safeArtifactPart(request.phaseId)}-${request.requestDigest.slice(0, 8)}`;
  await appendGithubEnv({
    V7_REQUEST_ID: request.requestId,
    V7_PHASE_ID: request.phaseId,
    V7_REQUEST_DIGEST: request.requestDigest,
    V7_ARTIFACT_NAME: artifactName,
    V7_EVIDENCE_DIR: evidenceRoot,
    V7_DISPOSITION: result.disposition,
  });
...
async function submitCommand(args) {
  const result = await submitV7Request({
    requestPath: args.request,
    repository: args.repository ?? 'CurveYield2/Contract-Automation',
    base: args.base ?? 'main',
    branch: typeof args.branch === 'string' ? args.branch : null,
    createPullRequest: args.pr !== false,
  });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
  return 0;
}

async function harnessInitCommand(args) {
  const result = await initializePhase6HarnessBundle({
    requestPath: args.request,
    runnerRoot: RUNNER_ROOT,
    campaign: args.campaign ?? 'property',
    type: args.type ?? 'standard',
    bundleId: typeof args.bundle === 'string' ? args.bundle : null,
  });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
  return 0;
}

async function harnessValidateCommand(args) {
  const result = await validatePhase6HarnessBundle({
    bundleId: args.bundle,
    runnerRoot: RUNNER_ROOT,
    requestPath: typeof args.request === 'string' ? args.request : null,
  });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
...
async function manifestCommand(args) {
  if (args.write === true) {
    const destination = await writeRunnerManifestV2({ runnerRoot: RUNNER_ROOT });
    process.stdout.write(`${JSON.stringify({ status: 'WROTE', destination }, null, 2)}\n`);
    return 0;
  }
  const result = await checkRunnerManifestV2({ runnerRoot: RUNNER_ROOT });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
...
main()
  ...
  .catch((error) => {
    console.error(JSON.stringify({ status: 'CLI_FAILURE', message: error?.message ?? String(error), kind: error?.kind ?? null }, null, 2));
    process.exitCode = 1;
  });

...
contract Phase6AuditWantV4 is ERC20 {
    constructor() ERC20("Phase6 Audit Want", "P6W") {}
    function mint(address to, uint256 amount) external { _mint(to, amount); }
}

contract Phase6AuditStrategyV4 is ICurveYieldVaultStrategy {
...
contract CyvlSdtVaultPhase6HarnessV4 {
    ...
    address internal constant MEDUSA_ACTOR_4 = address(0x50000);
    address internal constant MEDUSA_ACTOR_5 = address(0x60000);

    Phase6AuditWantV4 public immutable wantToken;
    CurveYieldVault public immutable vault;
    Phase6AuditStrategyV4 public immutable strategy;
    Phase6VaultActorV4[6] public actors;
    ...
    uint256 public referenceProbeCount;
    bool public referenceLastProbePassed = true;
    uint256 public referenceLastActualShares;
    uint256 public referenceLastNormativeShares;

    constructor() {
        wantToken = new Phase6AuditWantV4();
        vault = new CurveYieldVault("Phase6 Audit Vault", "p6VAULT", address(this), address(this), 18);
        strategy = new Phase6AuditStrategyV4(address(wantToken), address(vault));
        vault.setStrategy(address(strategy));
        for (uint256 i = 0; i < 6; i++) {
            actors[i] = new Phase6VaultActorV4(address(wantToken), address(vault), address(this));
    ...
    function _actorIndex(address sender) internal pure returns (uint256) {
        ...
        if (sender == MEDUSA_ACTOR_4) return 4;
        if (sender == MEDUSA_ACTOR_5) return 5;
        return uint160(sender) % 6;
    }

    function _recipientIndex(address seed) internal pure returns (uint256) {
        return uint160(seed) % 6;
    }
    ...
    function actionDeposit(uint96 rawAmount) external {
        ...
        Phase6VaultActorV4 actor = actors[_actorIndex(msg.sender)];
    ...
    function actionTransferShares(address recipientSeed, uint96 rawShares) external {
        transferCalls++;
        Phase6VaultActorV4 actor = actors[_actorIndex(msg.sender)];
        Phase6VaultActorV4 recipient = actors[_recipientIndex(recipientSeed)];
        uint256 shares = vault.balanceOf(address(actor));
        if (shares == 0) { revertedTransitions++; return; }
        uint256 amount = _bounded(uint256(rawShares), 1, shares);
    ...
    /// @dev P6.4 refinement: explicit share-transfer balance-neighbor dictionary.
    function actionBoundaryTransfer(address recipientSeed, uint8 rawClass) external {
        transferCalls++;
        Phase6VaultActorV4 actor = actors[_actorIndex(msg.sender)];
        Phase6VaultActorV4 recipient = actors[_recipientIndex(recipientSeed)];
        uint256 shares = vault.balanceOf(address(actor));
    ...
    function actorShareSum() public view returns (uint256 total) {
        for (uint256 i = 0; i < 6; i++) total += vault.balanceOf(address(actors[i]));
    }
    ...
    function _fixture() internal returns (Phase6AuditWantV4 token, CurveYieldVault freshVault, Phase6AuditStrategyV4 freshStrategy) {
        token = new Phase6AuditWantV4();
        freshVault = new CurveYieldVault("Phase6 Probe Vault", "p6PROBE", address(this), address(this), 18);
        freshStrategy = new Phase6AuditStrategyV4(address(token), address(freshVault));
    ...
    function _secondDepositShares(uint256 estimate) internal returns (uint256 shares) {
        (, CurveYieldVault freshVault, Phase6AuditStrategyV4 freshStrategy) = _fixture();
    ...
    /// @dev Independent normative model: absent realized yield, an equal second deposit must mint
    /// the same nominal shares. The reference does not call vault quote/accounting functions.
    function probeIndependentReferenceEqualDeposits(uint96 rawEstimate) external {
        uint256 estimate = _bounded(uint256(rawEstimate), 1, 100e18);
        (, CurveYieldVault freshVault, Phase6AuditStrategyV4 freshStrategy) = _fixture();
        freshVault.deposit(100e18);
        freshStrategy.setEstimate(estimate);
        uint256 beforeShares = freshVault.balanceOf(address(this));
        freshVault.deposit(100e18);
        referenceLastActualShares = freshVault.balanceOf(address(this)) - beforeShares;
        referenceLastNormativeShares = 100e18;
        referenceProbeCount++;
        referenceLastProbePassed = referenceLastActualShares == referenceLastNormativeShares;
    }

    function property_Process9_10_independent_reference_matches_exact_vault() external view returns (bool) {
        return referenceProbeCount == 0 || referenceLastProbePassed;
    }
}
...
contract CyvlSdtVaultPhase6FoundryV4 {
    struct FuzzSelector { address addr; bytes4[] selectors; }

    CyvlSdtVaultPhase6HarnessV4 public harness;

    function setUp() public {
        harness = new CyvlSdtVaultPhase6HarnessV4();
    }
    ...
    function targetSelectors() public view returns (FuzzSelector[] memory selectors_) {
        ...
        selectors[0] = CyvlSdtVaultPhase6HarnessV4.actionDeposit.selector;
        selectors[1] = CyvlSdtVaultPhase6HarnessV4.actionWithdraw.selector;
        selectors[2] = CyvlSdtVaultPhase6HarnessV4.actionWithdrawAll.selector;
        selectors[3] = CyvlSdtVaultPhase6HarnessV4.actionTransferShares.selector;
        selectors[4] = CyvlSdtVaultPhase6HarnessV4.actionSetEstimate.selector;
        selectors[5] = CyvlSdtVaultPhase6HarnessV4.actionDonateStrategy.selector;

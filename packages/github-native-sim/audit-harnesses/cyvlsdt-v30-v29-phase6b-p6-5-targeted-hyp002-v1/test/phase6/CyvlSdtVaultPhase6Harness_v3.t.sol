...
contract Phase6AuditWantV3 is ERC20 {
    constructor() ERC20("Phase6 Audit Want", "P6W") {}
    function mint(address to, uint256 amount) external { _mint(to, amount); }
}

contract Phase6AuditStrategyV3 is ICurveYieldVaultStrategy {
...
contract CyvlSdtVaultPhase6HarnessV3 {
    ...
    Phase6AuditWantV3 public immutable wantToken;
    CurveYieldVault public immutable vault;
    Phase6AuditStrategyV3 public immutable strategy;
    Phase6VaultActorV3[4] public actors;
    ...
    constructor() {
        wantToken = new Phase6AuditWantV3();
        vault = new CurveYieldVault("Phase6 Audit Vault", "p6VAULT", address(this), address(this), 18);
        strategy = new Phase6AuditStrategyV3(address(wantToken), address(vault));
        vault.setStrategy(address(strategy));
        for (uint256 i = 0; i < 4; i++) {
            actors[i] = new Phase6VaultActorV3(address(wantToken), address(vault), address(this));
    ...
    function actionDeposit(uint96 rawAmount) external {
        ...
        Phase6VaultActorV3 actor = actors[_actorIndex(msg.sender)];
    ...
    function actionTransferShares(address recipientSeed, uint96 rawShares) external {
        transferCalls++;
        Phase6VaultActorV3 actor = actors[_actorIndex(msg.sender)];
        Phase6VaultActorV3 recipient = actors[_recipientIndex(recipientSeed)];
        uint256 shares = vault.balanceOf(address(actor));
        if (shares == 0) { revertedTransitions++; return; }
        uint256 amount = _bounded(uint256(rawShares), 1, shares);
    ...
    function _fixture() internal returns (Phase6AuditWantV3 token, CurveYieldVault freshVault, Phase6AuditStrategyV3 freshStrategy) {
        token = new Phase6AuditWantV3();
        freshVault = new CurveYieldVault("Phase6 Probe Vault", "p6PROBE", address(this), address(this), 18);
        freshStrategy = new Phase6AuditStrategyV3(address(token), address(freshVault));
    ...
    function _secondDepositShares(uint256 estimate) internal returns (uint256 shares) {
        (, CurveYieldVault freshVault, Phase6AuditStrategyV3 freshStrategy) = _fixture();
...
contract CyvlSdtVaultPhase6FoundryV3 {
    struct FuzzSelector { address addr; bytes4[] selectors; }

    CyvlSdtVaultPhase6HarnessV3 public harness;

    function setUp() public {
        harness = new CyvlSdtVaultPhase6HarnessV3();
    }
    ...
    function targetSelectors() public view returns (FuzzSelector[] memory selectors_) {
        ...
        selectors[0] = CyvlSdtVaultPhase6HarnessV3.actionDeposit.selector;
        selectors[1] = CyvlSdtVaultPhase6HarnessV3.actionWithdraw.selector;
        selectors[2] = CyvlSdtVaultPhase6HarnessV3.actionWithdrawAll.selector;
        selectors[3] = CyvlSdtVaultPhase6HarnessV3.actionTransferShares.selector;
        selectors[4] = CyvlSdtVaultPhase6HarnessV3.actionSetEstimate.selector;
        selectors[5] = CyvlSdtVaultPhase6HarnessV3.actionDonateStrategy.selector;

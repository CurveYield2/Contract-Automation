...
contract Phase6AuditWantV1 is ERC20 {
    constructor() ERC20("Phase6 Audit Want", "P6W") {}
    function mint(address to, uint256 amount) external { _mint(to, amount); }
}

contract Phase6AuditStrategyV1 is ICurveYieldVaultStrategy {
...
contract CyvlSdtVaultPhase6HarnessV1 {
    ...
    Phase6AuditWantV1 public immutable wantToken;
    CurveYieldVault public immutable vault;
    Phase6AuditStrategyV1 public immutable strategy;
    Phase6VaultActorV1[4] public actors;
    ...
    constructor() {
        wantToken = new Phase6AuditWantV1();
        vault = new CurveYieldVault("Phase6 Audit Vault", "p6VAULT", address(this), address(this), 18);
        strategy = new Phase6AuditStrategyV1(address(wantToken), address(vault));
        vault.setStrategy(address(strategy));
        for (uint256 i = 0; i < 4; i++) {
            actors[i] = new Phase6VaultActorV1(address(wantToken), address(vault), address(this));
    ...
    function actionDeposit(uint96 rawAmount) external {
        ...
        Phase6VaultActorV1 actor = actors[_actorIndex(msg.sender)];
    ...
    function actionTransferShares(address recipientSeed, uint96 rawShares) external {
        transferCalls++;
        Phase6VaultActorV1 actor = actors[_actorIndex(msg.sender)];
        Phase6VaultActorV1 recipient = actors[_recipientIndex(recipientSeed)];
        uint256 shares = vault.balanceOf(address(actor));
        if (shares == 0) { revertedTransitions++; return; }
        uint256 amount = _bounded(uint256(rawShares), 1, shares);
    ...
    function _fixture() internal returns (Phase6AuditWantV1 token, CurveYieldVault freshVault, Phase6AuditStrategyV1 freshStrategy) {
        token = new Phase6AuditWantV1();
        freshVault = new CurveYieldVault("Phase6 Probe Vault", "p6PROBE", address(this), address(this), 18);
        freshStrategy = new Phase6AuditStrategyV1(address(token), address(freshVault));
    ...
    function _secondDepositShares(uint256 estimate) internal returns (uint256 shares) {
        (, CurveYieldVault freshVault, Phase6AuditStrategyV1 freshStrategy) = _fixture();
...
contract CyvlSdtVaultPhase6FoundryV1 {
    struct FuzzSelector { address addr; bytes4[] selectors; }

    CyvlSdtVaultPhase6HarnessV1 public harness;

    function setUp() public {
        harness = new CyvlSdtVaultPhase6HarnessV1();
    }
    ...
    function targetSelectors() public view returns (FuzzSelector[] memory selectors_) {
        ...
        selectors[0] = CyvlSdtVaultPhase6HarnessV1.actionDeposit.selector;
        selectors[1] = CyvlSdtVaultPhase6HarnessV1.actionWithdraw.selector;
        selectors[2] = CyvlSdtVaultPhase6HarnessV1.actionWithdrawAll.selector;
        selectors[3] = CyvlSdtVaultPhase6HarnessV1.actionTransferShares.selector;
        selectors[4] = CyvlSdtVaultPhase6HarnessV1.actionSetEstimate.selector;
        selectors[5] = CyvlSdtVaultPhase6HarnessV1.actionDonateStrategy.selector;

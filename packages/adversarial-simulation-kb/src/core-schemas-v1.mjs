...
export const CORE_REQUIRED_FIELDS_V1=Object.freeze({
  ...
  pattern:Object.freeze(['schemaVersion','patternId','name','description','rootCauseClass','structuralPreconditions','runtimePreconditions','attackerCapabilities','attackSequenceAbstract','expectedSecurityPropertyViolation','sourceIntelligenceSignals','runtimeOverlaySignals','nonApplicabilitySignals','falsePositiveGuards','adaptationRules','recommendedBackends','historicalIncidentRefs','recipeRefs','limitations']),
  recipe:Object.freeze(['schemaVersion','recipeId','patternRefs','targetTopologies','requiredTargetBindings','setupSteps','attackSteps','observations','assertions','adaptationInputs','backendSupport','executableRefs','proofStatusRef']),
  executable:Object.freeze(['schemaVersion','executableId','incidentRefs','patternRefs','recipeRefs','backend','toolVersion','language','sourceFiles','expectedSetup','requiredRpcForkState','requiredBlockIdentity','requiredImpersonation','requiredTokenBalances','expectedResult','proofStatusRef','lastQualification']),
...
export function validateCoreRecordV1(kind,record){
  ...
  if(kind==='incident'){
    ...
    if(!Number.isInteger(record.revision)||record.revision<1)errors.push({code:'REVISION'});
    ...
    if(!nonempty(record.language))errors.push({code:'LANGUAGE'}); for(const f of record.sourceFiles??[])if(!nonempty(f?.path)||!SHA64.test(f?.digest??''))errors.push({code:'SOURCE_FILE_IDENTITY'}); if(!SHA40.test(record.lastQualification?.commit??'')||!nonempty(record.lastQualification?.runId))errors.push({code:'LAST_QUALIFICATION'});
  }else if(kind==='proof'){

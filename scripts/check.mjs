...
if (forbiddenVersionedWorkflows.length > 0) {
  throw new Error(`Superseded/versioned V7 workflow entrypoints are forbidden: ${forbiddenVersionedWorkflows.join(', ')}`);
}
...
const staleManifests = processNames.filter((name) => /^RUNNER_MANIFEST_v\d+\.json$/.test(name));
if (staleManifests.length > 0) throw new Error(`Version-suffixed active runner manifests are forbidden: ${staleManifests.join(', ')}`);
...
console.log(`Syntax valid: ${files.length} JavaScript modules; V7 workflow/manifest/Anvil policy guards valid`);
